...
function Home() {
  ...
  //   setRange(newRange);
  //   audio.currentTime = (newRange / 100) * audio.duration;
  // }
  ...
  return (
    <div className="w-full h-screen bg-black flex relative overflow-hidden">
      <MdKeyboardArrowDown
        className={`absolute text-white top-[25px] left-[10%] text-[30px] md:hidden ${arrow ? 'rotate-[-90deg]' : null} cursor-pointer`}
        ...
        <>
          <div className="w-full  md:w-[50%] h-full flex justify-start items-center pt-[20px] md:pt-[120px] flex-col gap-[30px]">
            ...
            <div className="w-[50%] flex justify-center items-center relative rounded-md">
              ...
              />
              <div
                className={`bg-white h-[100%] rounded-md absolute left-0 `}
                ref={progress}
              ></div>
            </div>
           ...
           hover:text-gray-600  transition-all cursor-pointer "
                onClick={() => prevSong()}
              />

              {!playingSong ? (
                <div
                  className="w-[50px] h-[50px] rounded-full bg-white text-black flex justify-center items-center hover:bg-gray-500 transition-all cursor-pointer"
                  onClick={() => playSong()}
                >
                  <IoPlay className="w-[22px] h-[22px]" />
                </div>
              ) : (
                <div
                  className="w-[50px] h-[50px] rounded-full bg-white text-black flex justify-center items-center hover:bg-gray-500 transition-all cursor-pointer"
                  onClick={() => pauseSong()}
                >
                  <MdOutlinePause className="w-[22px] h-[22px]" />
                </div>
              )}
              <CgPlayTrackNext
          ...
          <div className="w-[100%] md:w-[50%] h-full hidden md:flex flex-col gap-5 pt-[120px] overflow-auto pb-[20px] items-center">
            {songsData.map((song) => (
              <Card
                key={song.id}
                name={song.name}
